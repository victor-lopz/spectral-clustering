...
def grafica_eigengaps_vs_radi_for_slides(
    result: SpectralAnalysisResult,
    params: ParametresGenerals,
    indexs_max_rel: list[int]|None = None,
    subfolder: str|None = None,
    ...
    """
    Es basa en la funció anterior, però ara genera 2 gràfiques
    simplificades:
    1. Només els eigengaps normalitzats.
    2. Eigengaps normalitzats + nombre de clústers.
    Treu els percentatges d'esparsificació i les línies verticals 
    dels estadístics de la matriu de pesos.
    """
    plt.rcParams.update({'font.size': 14})
    for grafica in ["eigengap", "eigengap_i_num_clusters"]:
        if grafica == "eigengap":
            fig, host = plt.subplots(figsize=(8.51, 6))
        else:
            fig, host = plt.subplots(figsize=(10, 6))
        fig.subplots_adjust(right=0.75)
        plt.rcParams.update({'font.size': 14})
        ...
        color_gap = 'tab:blue'
        color_clust = 'tab:red'
        p1, = host.plot(result.radis, result.normalized_eigengaps, marker='.', color=color_gap, label='Salt espectral normalitzat')
        p2, = par1.plot(result.radis, result.nums_clusters, marker='.', color=color_clust, label='Nombre de clústers')
        host.set_xlabel("Radi d'esparsificació")
        host.set_ylabel('Salt espectral normalitzat', color=color_gap)
        par1.set_ylabel('Nombre de clústers', color=color_clust)
        host.tick_params(axis='y', labelcolor=color_gap)
        par1.tick_params(axis='y', labelcolor=color_clust)
        host.yaxis.get_offset_text().set_horizontalalignment('left')
        ...
        if grafica == "eigengap":
            ...
            host.set_title(r"Salt espectral vs. radi d'esparsificació")
            lines = [p1]
        else:
            host.set_title(r"Salt espectral i clústers vs. radi d'esparsificació")
            lines = [p1, p2] 
        host.grid(True, alpha=0.3)
        labels: list[str] = [str(l.get_label()) for l in lines]
        for l in host.get_lines():
        ...
        host.legend(lines, labels, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=3)
        fig.tight_layout()
        filename = (f"slides-{grafica}_vs_radi-max_clusters={params.max_clusters}_radis={len(result.radis)}"
                    f"_t_end={params.t_span[1]:.1f}.pdf")
        plt.savefig(get_output_path(filename, subfolder), bbox_inches='tight')
        plt.show()